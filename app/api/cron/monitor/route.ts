...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const monitor of activeMonitors) {
      const result = await trackTerminalContainer(monitor.port, monitor.containerNo);
      ...
      if (result.success && result.status === "GNSTK") {
        ...
        processed.push({ containerNo: monitor.containerNo, status: "Updated to GNSTK" });
      } else {
        processed.push({ containerNo: monitor.containerNo, status: result.status || "Unchanged" });
      }
    }
