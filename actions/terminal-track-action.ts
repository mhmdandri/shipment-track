...
export async function trackTerminalContainer(
  ...
  try {
    ...
    if (normalizedPort === "ter3" || normalizedPort === "parama") {
      // 1. Check RAM cache first, if empty, load from DB
      if (!ter3SessionCache) {
        try {
          ...
          if (dbSession) {
            ter3SessionCache = JSON.parse(dbSession.value);
      ...
      const attemptTrack = async (session: typeof ter3SessionCache) => {
        if (!session) return null;

        const trackUrl = "https://parama.pelindo.co.id:8031/gateway-8021/api/parama/getContainerDetail";
      ...
      let trackData = await attemptTrack(ter3SessionCache);

      // If no valid session or track failed (e.g., expired session), do login
      if (!trackData || trackData.code !== "1") {
        console.log(`[TER3] Session empty or expired. Performing LOGIN for ${containerNo}...`);
        ...
        const rawSetCookies = loginRes.headers.getSetCookie
          ...
          : [];
        const cookieStr = rawSetCookies.map((c) => c.split(";")[0]).join("; ");
        ...
        try {
          await prisma.systemConfig.upsert({
            where: { key: "TER3_SESSION" },
            update: { value: JSON.stringify(ter3SessionCache) },
            create: { key: "TER3_SESSION", value: JSON.stringify(ter3SessionCache) },
          });
        ...
        // Retry track
        trackData = await attemptTrack(ter3SessionCache);
      }
      ...
      const rec = trackData.dataRec;
      let finalStatus = rec.statusCode || "UNKNOWN";
      let time = rec.updateTime || "";

      // Prioritize the latest handling activity if available
