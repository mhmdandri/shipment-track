...
- **Status Mapping Note (KOJA & NPCT1)**: Currently, tracking for KOJA and NPCT1 has only been tested using containers that have already outgated. Thus, their parser currently outputs the raw status string from the HTML. We do not yet know the exact string representation for ONVESSEL or GNSTK on these terminals. Future updates will require real data samples of containers still on vessel or newly stacked in the yard to map them correctly to `GNSTK`.
