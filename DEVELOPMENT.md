...
2.  Add a new rule to the `REMINDER_TEMPLATES` array:
    ...
    export const REMINDER_TEMPLATES = [
      {
        daysBeforeEta: 7,
        title: "Check Draft PIB",
        priority: "MEDIUM" as const,
      },
      {
        daysBeforeEta: 4,
        title: "Pre-book Trucking Co.",
...
### v1.12.0 � 2026-07-18: Terminal Container Tracking & Telegram Automation

**Feature**: Added real-time tracking for local terminals (JICT, TMAL) and an automated cron job system that pings via a serverless route to monitor yard allocations (GNSTK) and alert users via Telegram.

#### Improvements

1. **Terminal Tracking (Scraping & API)**: Implemented tracking logic inside \ctions/terminal-track-action.ts\ supporting JSON APIs (for JICT) and HTML scraping using \cheerio\ (for TMAL). All yard allocation milestones are normalized to \GNSTK\.
2. **Auto-Monitoring & Telegram Alerts**: Added \/api/cron/monitor\ route that checks monitored containers in the \TerminalMonitor\ database table. If a container receives a yard allocation (GNSTK), it sends an automated alert via Telegram Bot API using the configured \TELEGRAM_BOT_TOKEN\ and \TELEGRAM_CHAT_ID\.
3. **Consistent Shadcn Usage**: Audited the entire codebase to replace all raw HTML \<button>\ and \<input>\ elements with Shadcn UI \<Button>\ and \<Input>\ components, ensuring zero ESLint errors and strict TypeScript compliance (No \ny\).
